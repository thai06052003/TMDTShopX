...
namespace ShopxEX1.Services.Implementations
{
    public class OrderService : IOrderService
    {
        ...
        {
            "Chờ xác nhận", "Đang xử lý", "Đang giao", "Đã giao", "Đã hủy", "Đã hoàn tiền"
        };
        ...
        public async Task<List<OrderDto>> CreateOrderFromCartAsync(int userId, OrderCreateDto createDto)
        {
            ...
            using (var connection = CreateDbConnection())
            {
                ...
                var cartItemData = await connection.QueryAsync<CartItem, Cart, Product, Seller, CartItem>(
                    sqlQueryCartItems,
                    map: (cartItem, cart, product, seller) =>
                    {
                    ...
                    splitOn: "Dapper_SplitOn_Cart_CartID,Dapper_SplitOn_Product_ProductID,Dapper_SplitOn_Seller_SellerID"
                );
            ...
            try
            {
                ...
                {
                    // Nạp thủ công hoặc query lại với Include nếu cần
                    // Entry().Reference().LoadAsync() và Entry().Collection().LoadAsync()
                    // chỉ hoạt động tốt nếu thực thể đang được ChangeTracker theo dõi.
                    // Sau SaveChanges, chúng vẫn được theo dõi (trừ khi bạn Detach).

                    // Cách 1: Nạp thủ công (nếu orderEntity vẫn được theo dõi)
                    ...
                    // Sau khi nạp xong, bạn có thể Detach orderEntity nếu muốn dùng AsNoTracking cho AutoMapper
                    // _context.Entry(orderEntity).State = EntityState.Detached; (Tùy chọn)
        ...
        public async Task<PagedResult<OrderSummaryDto>> GetOrdersByUserIdAsync(int userId, OrderFilterDto filter, int pageNumber, int pageSize)
        {
            var query = _context.Orders
                                .Where(o => o.UserID == userId)
                                .Include(o => o.User) // Để lấy CustomerName cho OrderSummaryDto
                                .Include(o => o.OrderDetails) // Để tính NumberOfItems
                                .AsNoTracking();
        ...
        public async Task<bool> UpdateOrderStatusAsync(int orderId, OrderStatusUpdateDto statusUpdateDto, int userId, string userRole)
        {
            ...
            // Logic chuyển đổi trạng thái (ví dụ: không thể từ Đã giao về Chờ xác nhận)
            if (order.Status == "Đã giao" && statusUpdateDto.NewStatus != "Trả hàng" && statusUpdateDto.NewStatus != "Đã hoàn tiền")
            {
            ...
            if (order.Status == "Đã hủy" && statusUpdateDto.NewStatus != "Đã hủy") // Không thể thay đổi khi đã hủy
            {
            ...
            if (statusUpdateDto.NewStatus.Equals("Đã hủy", StringComparison.OrdinalIgnoreCase) &&
                (order.Status.Equals("Chờ xác nhận", StringComparison.OrdinalIgnoreCase) || order.Status.Equals("Đang xử lý", StringComparison.OrdinalIgnoreCase)))
            {
